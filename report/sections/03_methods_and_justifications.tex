...
while reconstructing it with minimal loss. Ultimately, it helps preventing the model from \textit{overfitting} the training dataset,
...
it into 3 chunks: training, validation and test. I used the validation set for \textbf{hyperparameter tuning},
to find the best set of model parameters that minimised the cost function. Also to regularise the model with \textbf{early stopping}.
...
The test set was finally used to assess the performances of the model using the best set of parameters.
...
parent programme. \textbf{Weight decay} and \textbf{batch normalisation} were tested during hyperparameter tuning and discarded for poor performances.

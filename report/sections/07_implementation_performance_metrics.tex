...
I did not use any residual-based loss function either because the change in the loss for wrong predictions
...
The network had the encoder and decoder had $N-1$ layers and the bottleneck layer in the middle.
If the value were $1$, the network would only have the bottleneck.

The number of nodes per layer was a positive integer $N > 0$. Unfortunately, the number of combinations was too high to be meaningfully optimised.
To reduce the complexity, I decided to couple the number of nodes per layer as a progression of integer divisions by 2.
...
(and \textit{vice versa} in the decoder), except for the layers adjacent to the input and output.
